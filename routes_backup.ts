...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  apiRouter.get("/job-platform/search", async (req, res) => {
    try {
      ...
      if (!service.hasValidCredentials(credentials)) {
      ...
      // Search for jobs
      const result = await service.searchJobs(credentials, searchParams);
  ...
  apiRouter.get("/job-platform/job-details", async (req, res) => {
    try {
      ...
      if (!jobListing) {
        const jobDetails = await service.getJobDetails(credentials, externalId);
  ...
  apiRouter.post("/job-platform/apply", async (req, res) => {
    try {
      ...
      // Apply to the job
      const result = await service.applyToJob(credentials, jobListing.externalId, {
        resumeId,
        coverId,
        customMessage
      });
